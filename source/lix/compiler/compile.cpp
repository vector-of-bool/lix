...
std::string make_error_whatstring(const std::string& what, lix::opt_ref<const ast::meta> meta) {
    if (meta) {
        using std::to_string;
...
lix::compile_error::compile_error(const std::string& what, lix::opt_ref<const lix::ast::meta> meta)
    : std::runtime_error(make_error_whatstring(what, meta))
    , _line(meta ? meta->line() : -1)
    , _column(meta ? meta->column() : -1) {}

code::code lix::compile(const ast::node& node) {
    lix::code::code_builder builder;
