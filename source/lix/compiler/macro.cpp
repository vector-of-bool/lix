...
ast::node lix::expand_macros(exec::context& ctx, const ast::node& input) {
    macro_expander ex{ctx};
    return input.visit(ex);
}

ast::node lix::escape(const ast::node& n) { return n.visit(ast_escaper{}); }

ast::node lix::escape(const lix::value& v) { return escape(node::from_value(v)); }