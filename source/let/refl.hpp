...
struct rt_meminfo_base {
    ...
    virtual let::value         get(const void* parent) const noexcept = 0;
};
...
struct rt_meminfo_impl : rt_meminfo_base {
    ...
    inline let::value   get(const void* parent) const noexcept override;
};
...
class rtti_impl : public rtti_base {
    ...
    const let::refl::type_id&  id() const noexcept override { return ct_info::id(); }
...
public:
    ...
    const let::refl::type_id& id() const noexcept { return _impl->id(); }
...
public:
    ...
    inline let::value get_unsafe(const void* parent) const noexcept;
};
...
template <>
struct conversions<let::integer> {
...
#define LET_REFL_DECLARE_MEMINFO(parent_type, idx, memname)                                        \
    ...
    struct member_info<parent_type, idx> {                                                         \
        ...
        constexpr static bool is_method                                                            \
            = ::let::refl::detail::is_method<std::decay_t<decltype(pointer)>>::value;              \
        constexpr static bool is_gettable                                                          \
            = ::let::refl::detail::is_gettable<std::decay_t<decltype(pointer)>>::value;            \
    };
...
#define LET_TYPEINFO(type_param, ...)                                                              \
    ...
    struct ct_base_type_info<type_param> {                                                         \
        ...
        constexpr static auto        member_count                                                  \
            ...
            - 1;                                                                                   \
        template <std::size_t N>                                                                   \
        using nth_member = ::let::refl::member_info<type_param, N>;                                \
        using members    = ::let::refl::detail::make_member_list_t<                                \
            void LET_MAP(LET_MEMBER_LIST_HELPER, ~, __VA_ARGS__)>;                              \
    };                                                                                             \
    }                                                                                              \
    static_assert(true)
