...
// Forward refs
namespace hamt {
    template <typename T> struct default_lookup;
    template<typename T, typename Lookup = default_lookup<T>> class branch_node;
    template<typename T, typename Lookup = default_lookup<T>> class leaf_node;
}
...
namespace std
{
    ...
    class default_delete<hamt::branch_node<T, Lookup>> {
    public:
        void operator()(hamt::branch_node<T, Lookup> *p);
    };

    template<typename T, typename Lookup>
    class default_delete<hamt::leaf_node<T, Lookup>> {
    public:
        void operator()(hamt::leaf_node<T, Lookup> *p);
    };
}
...
namespace hamt {
    ...
    template <typename T>
    struct default_lookup {
        static size_t hash(const T& what) { return std::hash<T>()(what); }
    ...
    public:
        ...
        template <typename U>
        auto with_replaced_value(size_t index, U &&newValue) const {
            auto newLeaf = create_unpopulated(m_size, m_hash);
            for( size_t i=0; i < m_size; ++i )
                if (i != index)
                    new (&newLeaf->m_values[i]) T( m_values[i] );
            new (&newLeaf->m_values[index]) T( std::forward<U>( newValue ) );
            return newLeaf;
        }

        auto with_erased_value(size_t index) const -> std::unique_ptr<leaf_node> {
            assert(index < m_size);
            if (m_size == 1) {
                // We're removing the only element. Don't create an empty leaf,
                // just return nothing.
                return nullptr;
            }
            auto newLeaf = create_unpopulated(m_size - 1, m_hash);
            for (size_t i = 0; i < index; ++i)
                new (&newLeaf->m_values[i]) T(m_values[i]);
            // Skip over the element at index
            for (size_t i = index + 1; i < m_size; ++i)
                new (&newLeaf->m_values[i - 1]) T(m_values[i]);
            return newLeaf;
        }
        ...
        template <typename U>
        auto index_of(U const& value) const -> size_t {
            for( size_t i=0; i < m_size; ++i )
                if( Lookup::compare(m_values[i], value) )
                    return i;
            return npos;
        }

        template <typename U>
        auto find( U const& value ) const -> T const* {
            auto idx = index_of(value);
            if (idx != npos)
                return std::addressof(get_at(idx));
            return nullptr;
        }
        ...
        ~branch_node() {
            ...
            for( size_t i = 0; i < len; ++i ) {
                ...
                if( node->m_type == node_type::branch )
                    release( static_cast<branch_node<T, Lookup> const*>( node ) );
                else
                    release( static_cast<leaf_node<T, Lookup> const*>( node ) );
            }
        }
    ...
    public:
        ...
        static auto create_pair(sparse_index index1, leaf_node<T, Lookup> const *leaf1, sparse_index index2,
                                leaf_node<T, Lookup> const *leaf2) -> std::unique_ptr<branch_node> {
        ...
        auto with_replaced(sparse_index sparseIndex, node const *child) const -> std::unique_ptr<branch_node> {
            auto originalSize = size();
            auto removing     = child == nullptr;
            auto newSize      = removing ? originalSize - 1 : originalSize;
            auto newBits      = removing ? m_bitmap ^ sparseIndex.bit_position() : m_bitmap;

            // If replacing a node we overwrite existing in place
            assert( ( m_bitmap & sparseIndex.bit_position() ) != 0 );

            auto node = create_unpopulated( newSize, newBits );
            ...
            auto copyBegin = removing ? splitPoint : splitPoint + 1;
            for (size_t i = copyBegin; i < newSize; ++i) {
                auto sharedNode = node->m_children[i] = m_children[i];
                addref(sharedNode);
            }
            return node;
    ...
    class iterator {
        struct Level {
            branch_node<T, Lookup> const *branch;
            size_t compactIndex;
            size_t width;
        };
        Level m_levels[detail::maxDepth];

        leaf_node<T, Lookup> const* m_leaf;

        size_t m_depth;

        void descend_from(branch_node<T, Lookup> const *branch, size_t depth) {
            leaf_node<T, Lookup> const* leaf = nullptr;
            while( !leaf ) {
    ...
    public:
        explicit iterator( branch_node<T, Lookup> const* root ) {
            if( root ) {
                descend_from(root, 0);
            }
            else {
        ...
        auto operator++() -> iterator& {
            ...
            assert( nextNode );
            if( nextNode->m_type == node_type::leaf )
                m_leaf = static_cast<leaf_node<T, Lookup> const*>( nextNode );
            else
                descend_from(static_cast<branch_node<T, Lookup> const *>( nextNode ), m_depth);
    ...
    template<typename T, typename Lookup = default_lookup<T>, typename U = T>
    class path {

        branch_node<T, Lookup> const *m_branches[detail::maxDepth];
        size_t m_chunks[detail::maxDepth];

        branch_node<T, Lookup> const *m_lastBranch;
        detail::chunked_hash m_chunkedHash;
        leaf_node<T, Lookup> const* m_leaf;
        size_t m_size = 0;

    public:
        path( U const& value, branch_node<T, Lookup> const* root ) : m_chunkedHash( Lookup::hash( value ) ) { // NOLINT
            size_t size = 0;
            assert( root != nullptr );
            ...
            while( nextNode && nextNode->m_type == node_type::branch ) {
                ...
                lastBranch = static_cast<branch_node<T, Lookup> const*>( nextNode );
        ...
        auto rewrite( branch_node<T, Lookup> const* newBranch ) const -> branch_node<T, Lookup> const* {
    ...
    template<typename T, typename Lookup = default_lookup<T>>
    struct hash_trie_data {
        branch_node<T, Lookup> const* m_root;
        size_t m_size;
    };

    template<typename T, typename Lookup, typename PathKey>
    auto add_value_at_currently_unset_position(path<T, Lookup, PathKey> const &path, std::unique_ptr<leaf_node<T, Lookup>> &&leaf) {
    ...
    auto extend
            (   detail::chunked_hash existingHash,
                leaf_node<T, Lookup> const *existingLeaf,
                detail::chunked_hash newHash,
                std::unique_ptr<leaf_node<T, Lookup>> &&newLeaf ) -> std::unique_ptr<branch_node<T, Lookup>> {
        if( existingHash.chunk == newHash.chunk ) {
            auto newChildBranch = extend(existingHash + 1, existingLeaf, newHash + 1, std::move(newLeaf));
            auto newParentBranch = branch_node<T, Lookup>::create_single(sparse_index(newHash.chunk), newChildBranch.get());
            newChildBranch.release();
            return newParentBranch;
        }
        else { // NOLINT
            auto newBranch = branch_node<T, Lookup>::create_pair(sparse_index(existingHash.chunk), existingLeaf,
    ...
    template<typename U, typename T, typename Lookup, typename PathKey>
    static auto add_value_at_leaf(path<T, Lookup, PathKey> const &path, U &&value) -> branch_node<T, Lookup> const* {
        ...
        // Different hash, so add branches to the point they diverge
            ...
            auto newChildBranch = extend
                    ( existingHash + 1,
                      existingLeaf,
                      path.chunked_hash() + 1,
                      leaf_node<T, Lookup>::create(std::forward<U>(value), path.whole_hash()));
            auto newBranch = path.last_branch()->with_replaced
                    (sparse_index(path.hash_chunk()),
                     newChildBranch.get());
            auto newRoot = path.rewrite(newBranch.get());
    ...
    template<typename U, typename T, typename Lookup>
    static auto inserted( branch_node<T, Lookup> const* root, U &&value) -> branch_node<T, Lookup> const* {
        ...
        return path.leaf()
               ...
               : add_value_at_currently_unset_position
                       ( path, leaf_node<T, Lookup>::create(std::forward<U>(value), path.whole_hash() ) );
    }
    ...
    class hash_trie {
        ...
        hash_trie_data<T, Lookup> m_data;
        friend shared_hash_trie<T, Lookup>;

        static auto makeEmptyData() -> hash_trie_data<T, Lookup> {
            return { branch_node<T, Lookup>::create_empty().release(), 0 };
        }

    public:
        hash_trie() : m_data( makeEmptyData() ) {}

        explicit hash_trie( hash_trie_data<T, Lookup> const& data ) : m_data( data ) {
            addref( m_data.m_root );
        }
        hash_trie( hash_trie<T, Lookup> const& other ) : hash_trie( other.m_data ) {}
        hash_trie( hash_trie<T, Lookup>&& other ) noexcept(false) : hash_trie() {
            swap( other );
        }
        ...
        auto begin() -> iterator<T, Lookup> {
            return iterator<T, Lookup>( m_data.m_root );
        }
        auto end() -> iterator<T, Lookup> {
            return iterator<T, Lookup>( nullptr );
        }

        auto data() const -> hash_trie_data<T, Lookup> const& { return m_data; }
        auto data() -> hash_trie_data<T, Lookup>& { return m_data; }
    };
    ...
    class shared_hash_trie { // NOLINT
        static_assert( std::is_trivially_copyable<hash_trie_data<T, Lookup>>::value,
                       "hash_trie_data must be trivially copyable to be used atomically" );

        std::atomic<hash_trie_data<T, Lookup>> m_data;
    ...
    public:
        ...
        explicit shared_hash_trie( hash_trie<T, Lookup> const& hash_trie ) {
            m_data.store( hash_trie.data(), std::memory_order_relaxed );
            addref( hash_trie.data().m_root );
        }

        auto data() const -> hash_trie_data<T, Lookup> {
            return m_data.load( std::memory_order_relaxed );
        }

        auto get() const -> hash_trie<T, Lookup> {
            return hash_trie<T, Lookup>( data() );
        }

        auto start_transaction() -> hash_trie_transaction<T, Lookup>;

        template<typename L>
        void update_with(L const &updateTask);

        // "low level" compare-exchange wrapper - use transaction
        auto reset( hash_trie_data<T, Lookup>& originalData,
                    hash_trie_data<T, Lookup>& newData ) -> bool {
    ...
    template<typename T, typename Lookup>
    class hash_trie_transaction {
        hash_trie_data<T, Lookup> m_baseData; // For compare-exchange
        shared_hash_trie<T, Lookup>& m_shared;

    public:
        explicit hash_trie_transaction( shared_hash_trie<T, Lookup>& shared )
        ...
        auto get() const -> hash_trie<T, Lookup> {
            return hash_trie<T, Lookup>( m_baseData );
        }

        auto try_commit(hash_trie<T, Lookup> &newHashTrie) -> bool {
        ...
        void update_with(L const &updateTask) {
            while( true ) {
                hash_trie<T, Lookup> copy( m_baseData );
                updateTask( copy );
    ...
    template<typename T, typename Lookup>
    auto shared_hash_trie<T, Lookup>::start_transaction() -> hash_trie_transaction<T, Lookup> {
        return hash_trie_transaction<T, Lookup>( *this );
    }
    template<typename T, typename Lookup>
    template<typename L>
    void shared_hash_trie<T, Lookup>::update_with(L const &updateTask) {
...
{
    template<typename T, typename Lookup>
    void default_delete<hamt::branch_node<T, Lookup>>::operator()( hamt::branch_node<T, Lookup> *p ) {
        p->~branch_node();
    ...
    template<typename T, typename Lookup>
    void default_delete<hamt::leaf_node<T, Lookup>>::operator()( hamt::leaf_node<T, Lookup> *p ) {
        p->~leaf_node();
        auto rawStorage = reinterpret_cast<unsigned char*>( p ); // NOLINT
        delete[] rawStorage;
    }
